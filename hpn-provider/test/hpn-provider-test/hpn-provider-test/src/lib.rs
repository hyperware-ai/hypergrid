...
use caller_utils::{HpnProviderState, TodoItem};
...
async_test_suite!(
    "hpn-provider-test-template-dot-os-v0",
    ...
    test_import_export_state: async {
        let address: Address = ("hpn-provider.os", "hpn-provider", "hpn-provider", "template.os").into();

        // 1. Define initial state (dummy data)
        let initial_state = HpnProviderState {
            tasks: vec![
