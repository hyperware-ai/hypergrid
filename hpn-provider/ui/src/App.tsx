...
import "./App.css";
import useHpnProviderStore from "./store/hpn_provider";
import {
  HttpMethod,
  EndpointDefinition,
  RegisteredProvider,
...
function App() {
  const { registeredProviders, setRegisteredProviders } = useHpnProviderStore();
  ...
  const handleCopyFormData = useCallback(async () => {
    ...
    const hnsName = (providerName.trim() || "[YourProviderName]") + ".hpn-beta.hypr";
