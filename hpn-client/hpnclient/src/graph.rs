...
pub fn build_hpn_graph_data(
    our: &Address,
    state: &State,
    db: &Sqlite,
) -> anyhow::Result<HpnGraphResponse> {
    info!("Building HPN graph data for node {}...", our.node);
    ...
    Ok(HpnGraphResponse { nodes, edges })
}

pub fn handle_get_hpn_graph_layout(
    our: &Address,
    state: &State,
    db: &Sqlite,
) -> anyhow::Result<()> {
    info!("Handling GET /api/hpn-graph for node {}...", our.node);
    match build_hpn_graph_data(our, state, db) {
        Ok(graph_response) => {
            // Log the serialized JSON before sending
            match serde_json::to_string_pretty(&graph_response) {
                Ok(json_string) => {
                    info!("Serialized HpnGraphResponse JSON:\n{}", json_string);
                }
                Err(e) => {
                    error!("Failed to serialize HpnGraphResponse for logging: {:?}", e);
                }
            }
            send_json_response(StatusCode::OK, &graph_response)
        }
        Err(e) => {
            error!("Error building HPN graph data: {:?}", e);
