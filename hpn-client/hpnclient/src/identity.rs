...
/// Checks for the expected HPN sub-entry (e.g., hpn-wallet.<node_name>)
...
pub fn initialize_operator_identity(our: &Address, state: &mut State) -> Result<()> {
    info!("Initializing HPN Operator Identity for node: {} (Note: initialize_operator_identity might be deprecated for status checks)", our.node);
    ...
    match identity_status {
        ...
        _ => { // Handles NotFound, IncorrectImplementation, CheckError etc.
            let expected_sub_entry_name = format!("hpn-beta-wallet.{}", our.node);
            let expected_implementation_str = "0x000000000046886061414588bb9F63b6C53D8674"; // Duplicated, consider const
         error!("---------------------------------------------------------------------");
         error!("HPN OPERATIONAL SUB-ENTRY NOT FOUND OR INCORRECTLY CONFIGURED!");
         ...
         error!("Payments and other HPN operations requiring TBA interaction will fail.");
         error!("---------------------------------------------------------------------");
         ...
         if state.operator_entry_name.is_some() || state.operator_tba_address.is_some() {
                // Clear if *any* non-verified status occurs
                info!("Clearing potentially invalid operator identity state due to verification failure.");
...
pub fn check_operator_identity_detailed(our: &Address) -> IdentityStatus {
    info!("Checking detailed HPN Operator Identity status for node: {}", our.node);

    let base_node_name = our.node.clone();
    let expected_sub_entry_name = format!("hpn-beta-wallet.{}", base_node_name);
    ...
    match hypermap_reader.get(&expected_sub_entry_name) {
        Ok((tba, owner, _data)) => {
            if tba == EthAddress::ZERO {
                info!(
                    "HPN sub-entry '{}' lookup returned zero address TBA, interpreting as NOT FOUND.",
