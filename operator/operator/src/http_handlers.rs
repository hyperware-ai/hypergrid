...
fn deserialize_request(body: &[u8]) -> anyhow::Result<HttpServerRequest> {
    serde_json::from_slice(body).map_err(|e| {
        ...
        send_response(
            ...
            None,
            b"Invalid request format".to_vec(),
        );
...
fn route_http_request(
    ...
    match (method.clone(), path.as_str()) {
        ...
        (Method::POST, "/api/configure-authorized-client") => {
            handle_configure_client_route(our, req, state, db)
        }
...
fn handle_shim_mcp_route(
    ...
    match authenticate_shim_with_headers(state, client_id.clone(), token) {
        Ok(client_config) => {
            ...
            let updated_client_config = state
                .authorized_clients
                .get(client_id.as_ref().unwrap())
                .cloned()
                .unwrap();

            handle_post(our, state, db, Some(updated_client_config))
        }
        Err(auth_error) => handle_shim_auth_error(auth_error),
    }
}

fn handle_api_actions_route(_our: &Address, state: &mut State, _db: &Sqlite) -> anyhow::Result<()> {
...
fn handle_shim_auth_error(auth_error: AuthError) -> anyhow::Result<()> {
    let (status, message) = match auth_error {
        ...
        AuthError::InsufficientCapabilities => {
            (StatusCode::FORBIDDEN, "Client lacks necessary capabilities")
        }
    };
    ...
    match serde_json::from_slice::<ApiRequest>(blob.bytes()) {
        Ok(req) => {
            ...
            match req {
                ...
                ApiRequest::SetClientLimits { client_id, limits } => {
                    handle_set_client_limits(state, client_id, limits)
                }
                ApiRequest::ExportSelectedPrivateKey { password } => {
                    handle_export_private_key(state, password)
                }
                ApiRequest::SetSelectedWalletPassword {
                    new_password,
                    old_password,
                } => handle_set_wallet_password(state, new_password, old_password),
                ApiRequest::RemoveSelectedWalletPassword { current_password } => {
                ...
                ApiRequest::WithdrawUsdcFromOperatorTba {
                    to_address,
                    amount_usdc_units_str,
                } => handle_withdraw_usdc(state, to_address, amount_usdc_units_str),

                // Authorized client management
                ApiRequest::RenameAuthorizedClient {
                    client_id,
                    new_name,
                } => handle_rename_authorized_client(state, client_id, new_name),
                ApiRequest::DeleteAuthorizedClient { client_id } => {
                    handle_delete_authorized_client(state, client_id)
                }

                // ERC-4337 configuration
                ApiRequest::SetGaslessEnabled { enabled } => {
                    handle_set_gasless_enabled(state, enabled)
                }
            }
        }
...
fn handle_legacy_mcp(
    ...
    match req {
        ...
        HttpMcpRequest::GetWalletSummaryList {} => handle_get_wallet_summary_list(state),
        HttpMcpRequest::SelectWallet { wallet_id } => handle_select_wallet(state, wallet_id),
        HttpMcpRequest::RenameWallet {
            wallet_id,
            new_name,
        } => handle_rename_wallet(state, wallet_id, new_name),
        HttpMcpRequest::DeleteWallet { wallet_id } => handle_delete_wallet(state, wallet_id),
        HttpMcpRequest::GenerateWallet {} => handle_generate_wallet(state),
        HttpMcpRequest::ImportWallet {
            private_key,
            password,
            name,
        } => handle_import_wallet(state, private_key, password, name),
        HttpMcpRequest::ActivateWallet { password } => handle_activate_wallet(state, password),
        HttpMcpRequest::DeactivateWallet {} => handle_deactivate_wallet(state),
        HttpMcpRequest::SetWalletLimits { limits } => handle_set_wallet_limits(state, limits),
        HttpMcpRequest::ExportSelectedPrivateKey { password } => {
            handle_export_private_key(state, password)
        }
        HttpMcpRequest::SetSelectedWalletPassword {
            new_password,
            old_password,
        } => handle_set_wallet_password(state, new_password, old_password),
        ...
        HttpMcpRequest::GetActiveAccountDetails {} => handle_get_active_account_details(state),

        // Withdrawal operations
        HttpMcpRequest::WithdrawEthFromOperatorTba {
            to_address,
            amount_wei_str,
        } => handle_withdraw_eth(state, to_address, amount_wei_str),
        HttpMcpRequest::WithdrawUsdcFromOperatorTba {
            to_address,
            amount_usdc_units_str,
        } => handle_withdraw_usdc(state, to_address, amount_usdc_units_str),
    }
...
fn handle_get(
    ...
    match path_str {
        ...
        // Graph visualization
        "/api/hypergrid-graph" | "/hypergrid-graph" => {
            handle_get_hypergrid_graph_layout(our, state)
        }
    ...
    if let Some(db) = &state.db_conn {
        ...
        for rec in &mut rows {
            let tx_opt = match &rec.payment_result {
                Some(crate::structs::PaymentAttemptResult::Success { tx_hash, .. }) => {
                    Some(tx_hash.clone())
                }
                _ => None,
            };
                ...
                if let Ok(rs) = db.read(q, vec![serde_json::Value::String(tx.clone())]) {
                    if let Some(row) = rs.get(0) {
                        if let Some(units_str) =
                            row.get("total_cost_units").and_then(|v| v.as_str())
                        {
                            if let Ok(units_i) = units_str.parse::<i128>() {
                                ...
                                if let Some(existing) = &rec.response_json {
                                    if let Ok(e) =
                                ...
                                if let Some(crate::structs::PaymentAttemptResult::Success {
                                    amount_paid,
                                    ..
                                }) = &mut rec.payment_result
                                {
                                    *amount_paid =
                                        format!("{}", whole as f64 + (frac as f64) / 1_000_000.0);
                                }
                            }
                        }
                    }
...
fn handle_get_linked_wallets(state: &mut State) -> anyhow::Result<()> {
    ...
    let on_chain_wallets = if let Some(operator_entry_name) = &state.operator_entry_name {
        match hyperwallet_service::get_all_onchain_linked_hot_wallet_addresses(Some(
            operator_entry_name,
        )) {
    ...
    for on_chain_address in &on_chain_wallets {
        let is_managed = managed_summaries
            .iter()
            .any(|s| &s.address == on_chain_address);
        if !is_managed {
            linked_wallets.push(json!({
                ...
                if let Some(row) = rs.get(0) {
                    if let Some(units_str) = row.get("total_cost_units").and_then(|v| v.as_str()) {
                        ...
                        if let Ok(units_i) = units_str.parse::<i128>() {
                            ...
                            let mut extra = serde_json::json!({});
                            if let Some(existing) = &rec.response_json {
                                if let Ok(e) = serde_json::from_str::<serde_json::Value>(existing) {
                                    if e.is_object() {
                                        extra = e;
                                    }
                                }
...
fn handle_set_wallet_limits(state: &mut State, limits: SpendingLimits) -> anyhow::Result<()> {
    ...
    match hyperwallet_service::set_wallet_spending_limits(
        state,
        wallet_id,
        max_per_call,
        max_total,
...
fn handle_set_client_limits(
    ...
    state
        .client_limits_cache
        .insert(client_id.clone(), limits.clone());
    state.save();
...
fn handle_export_private_key(state: &State, password: Option<String>) -> anyhow::Result<()> {
    ...
    match hyperwallet_service::export_private_key(state, wallet_id, password) {
        Ok(private_key) => send_json_response(
            StatusCode::OK,
            &json!({
                "success": true,
...
fn handle_remove_wallet_password(
    state: &mut State,
...
fn handle_withdraw_eth(
    ...
    info!(
        "Withdrawing ETH to: {}, amount: {} wei",
        to_address, amount_wei_str
    );
...
fn handle_withdraw_usdc(
    state: &mut State,
    to_address: String,
    amount_usdc_units_str: String,
) -> anyhow::Result<()> {
    info!(
        "Withdrawing USDC to: {}, amount: {} units",
        to_address, amount_usdc_units_str
    );
    match hyperwallet_payments::handle_operator_tba_withdrawal(
        state,
...
fn determine_ui_payment_wallet(state: &State) -> Result<String, PaymentAttemptResult> {
    ...
    if !signer
        .address()
        .to_string()
        .eq_ignore_ascii_case(selected_id)
    {
        error!(
            "Selected wallet {} doesn't match active signer {}",
            selected_id,
            signer.address()
        );
...
fn limit_call_history(state: &mut State) {
    ...
    if state.call_history.len() > max_history {
        state
            .call_history
            .drain(..state.call_history.len() - max_history);
    }
}
...
pub fn handle_authorize_shim_request(
    ...
    let cookies_header_str: Option<String> = req
        .headers()
        .get("cookie")
    ...
    let token_value =
        extracted_token_value.unwrap_or_else(|| "PLACEHOLDER_TOKEN_VALUE".to_string());
    ...
    if node_name.starts_with("PLACEHOLDER") || token_value.starts_with("PLACEHOLDER") {
        ...
        return Ok(HttpResponse::builder()
            ...
            .body(
                r#"{"error": "Internal server error: Could not retrieve authentication details."}"#
                    .to_string()
                    .into_bytes(),
            )?);
    }
    ...
    let json_string = match serde_json::to_string_pretty(&config_data) {
        ...
        Err(e) => {
            ...
            return Ok(HttpResponse::builder()
                ...
                .header("Content-Type", "application/json")
                .body(
                    r#"{"error": "Failed to serialize configuration data."}"#
                        .to_string()
                        .into_bytes(),
                )?);
        }
    ...
    match vfs::create_file(&vfs_file_path, None) {
        Ok(file) => match file.write(json_string.as_bytes()) {
            ...
            Err(e) => {
                ...
                Ok(HttpResponse::builder()
                    ...
                    .header("Content-Type", "application/json")
                    .body(
                        r#"{"error": "Failed to write configuration file to VFS."}"#
                            .to_string()
                            .into_bytes(),
                    )?)
            }
        ...
        Err(e) => {
            error!(
                "Failed to create/open VFS tmp file {}: {:?}",
                vfs_file_path, e
            );
            Ok(HttpResponse::builder()
                .status(StatusCode::INTERNAL_SERVER_ERROR)
                .header("Content-Type", "application/json")
                .body(
                    r#"{"error": "Failed to create/open configuration file in VFS."}"#
                        .to_string()
                        .into_bytes(),
                )?)
        }
...
pub fn handle_configure_authorized_client(
        ...
        for cookie_pair in cookies.split(';') {
            ...
            if parts.len() == 2 {
                ...
                if name == expected_cookie_name {
                    ...
                    info!(
                        "Configure Client: Request authenticated for node owner: {}",
                        our.node()
                    );
                    break;
                }
    ...
    if !is_authenticated_owner {
        error!(
            "Configure Client: Request not authenticated as node owner. Cookie header: {:?}",
            cookies_header_str
        );
        return Ok(HttpResponse::builder()
            ...
            .header("Content-Type", "application/json")
            .body(
                r#"{"error": "Authentication error: Not authorized as node owner."}"#
                    .to_string()
                    .into_bytes(),
            )?);
    }
    ...
    let blob = last_blob().ok_or(anyhow::anyhow!(
        "Request body is missing for configure client request"
    ))?;
    let request_data: ConfigureAuthorizedClientRequest = match serde_json::from_slice(blob.bytes())
    {
        ...
        Err(e) => {
            error!(
                "Configure Client: Failed to deserialize request body: {}",
                e
            );
            return Ok(HttpResponse::builder()
    ...
    let (client_id, is_update) = if let Some(existing_id) = request_data.client_id {
        ...
        if let Some(existing_client) = state.authorized_clients.get_mut(&existing_id) {
            ...
            error!(
                "Configure Client: Client ID {} not found for update",
                existing_id
            );
            return Ok(HttpResponse::builder()
        ...
        let default_name = if request_data.hot_wallet_address_to_associate.len() >= 10 {
            format!(
                ...
                &request_data.hot_wallet_address_to_associate
                    [request_data.hot_wallet_address_to_associate.len() - 4..]
            )
        } else {
            format!(
                "Shim Client {}",
                new_client_id.chars().take(8).collect::<String>()
            )
        };
        ...
        state
            .authorized_clients
            .insert(new_client_id.clone(), new_client);
        (new_client_id, false)
    };
...
fn handle_payment(
    ...
    if price_f64 <= 0.0 {
        info!(
            "No payment required (Price: {} is zero or invalid).",
    ...
    if let Some(cfg) = client_config_opt {
        ...
        if let (Some(db), Some(tba)) = (state.db_conn.as_ref(), state.operator_tba_address.as_ref())
        {
            ...
            if let Ok(rows) = db.read(
                q,
                vec![
                    serde_json::Value::String(tba.to_lowercase()),
                    serde_json::Value::String(cfg.id.clone()),
                ],
            ) {
    ...
    info!(
        "Attempting payment of {} to {} for provider {} using wallet {}",
        provider_details.price_str,
    ...
    match payment_result {
        ...
        None => PaymentResult::Failed(PaymentAttemptResult::Skipped {
...
fn handle_rename_authorized_client(
    state: &mut State,
    client_id: String,
    new_name: String,
) -> anyhow::Result<()> {
...
// ===========================================================================================
// SPIDER INTEGRATION HANDLERS
// ===========================================================================================

fn handle_spider_connect(our: &Address, state: &mut State) -> anyhow::Result<()> {
    info!("Handling spider connect request");
    
    // Check if request wants to force a new key
    let force_new = if let Some(blob) = last_blob() {
        if let Ok(request) = serde_json::from_slice::<serde_json::Value>(blob.bytes()) {
            request.get("force_new").and_then(|v| v.as_bool()).unwrap_or(false)
        } else {
            false
        }
    } else {
    ...
    if !force_new {
        if let Some(existing_key) = &state.spider_api_key {
    ...
    let result: Result<crate::structs::SpiderApiKey, String> =
    ...
    match result {
        Ok(api_key) => {
            // Store the API key
            state.spider_api_key = Some(api_key.key.clone());
            state.save();
        ...
        Err(e) => {
            ...
            send_json_response(
                StatusCode::INTERNAL_SERVER_ERROR,
                &json!({
                    "error": format!("Failed to create spider API key: {}", e)
                }),
            )
        }
    }
...
fn handle_spider_chat(our: &Address, state: &mut State) -> anyhow::Result<()> {
    info!("Handling spider chat request");

    let blob = last_blob().ok_or(anyhow::anyhow!("Request body is missing"))?;
    let mut request: crate::structs::SpiderChatRequest = serde_json::from_slice(blob.bytes())
        .map_err(|e| anyhow::anyhow!("Failed to parse chat request: {}", e))?;
    ...
    for attempt in 1..=2 {
        ...
        if let Some(error_str) = chat_response.get("Err").and_then(|v| v.as_str()) {
            if error_str.contains("Unauthorized: Invalid API key") && attempt == 1 {
                ...
                let key_request = crate::structs::CreateSpiderKeyRequest {
                    name: format!("operator-{}", our.node),
                ...
                let key_result: Result<crate::structs::SpiderApiKey, String> = 
                    serde_json::from_slice(key_response_body)
                        .map_err(|e| anyhow::anyhow!("Failed to parse spider key response: {:?}", e))?;
                ...
                match key_result {
                    ...
                    Err(e) => {
                        ...
                        return send_json_response(
                            StatusCode::INTERNAL_SERVER_ERROR,
                            &json!({
                                "error": format!("Failed to refresh API key: {}", e),
                                "needs_reconnect": true
                            }),
                        );
                    }
        ...
        if let Some(ok_response) = chat_response.get("Ok") {
            // If we refreshed the key, return it in the response so frontend can update
            let mut response = json!({
                "conversationId": ok_response.get("conversationId").and_then(|v| v.as_str()).unwrap_or(""),
                "response": ok_response.get("response"),
                "allMessages": ok_response.get("allMessages"),
            });
            
            // Include the new API key if it was refreshed
            if attempt == 2 {
                response["refreshedApiKey"] = json!(request.api_key);
            }

            return send_json_response(StatusCode::OK, &response);
        }
    }
    ...
    send_json_response(
        StatusCode::INTERNAL_SERVER_ERROR,
        &json!({
            "error": "Invalid response from spider after retries"
        }),
    )
}
