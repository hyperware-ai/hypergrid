...
use hyperware_process_lib::{
    ...
    wallet::{self, create_erc20_transfer_calldata},
    hyperwallet_client,
};
...
pub fn handle_operator_tba_withdrawal(
    ...
    let (operation, params) = match asset_type {
        ...
        AssetType::Usdc => {
            ...
            // Use wallet library to encode the ERC20 transfer call
            let recipient_addr = to_address.parse::<EthAddress>()
                .map_err(|_| "Invalid recipient address".to_string())?;
            let call_data_bytes = create_erc20_transfer_calldata(recipient_addr, U256::from(amount_units));
            let call_data = format!("0x{}", hex::encode(call_data_bytes));
